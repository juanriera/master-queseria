...
## Visualización de Datos en Python y R
...
```{python}
sns.boxplot(data=df, y='est', color='lightgray')
...
ggplot(df, aes(x = "", y = est)) +
  ...
  geom_beeswarm(color = "darkblue", size = 2, cex = 1.5, alpha = 0.7) +
  labs(
...
Excel no permite la representación de los gráficos de densidad; en `Python`lo hemos hecho arriba hacerlo con `seaborn`, añadiendo la funcion de densidad a un histograma añadiendo la opción `kde = True`.
...
```{python}
plt.figure(figsize=(10.0, 2.0))
df["est"].resample('W-MON').std().plot(
...
```{python}
plt.figure(figsize=(10.0, 2.0))
df["est"]["2020-05":"2020-08"].resample('W-MON').mean().plot(
    title="Media del extracto seco total semanal")
plt.show()
```
...
A continuación, el gráfico realizado con R. Se recomienda copiar el código y pegarlo en un programa de IA para explicarlo en detalle.
...
El enlace https://tidyplots.org/ tiene muchos gráficos útile que puede resultar interesante y formativo ver con atención.

También esta guía proporciona información sobre el destino adecuado de cada tipo de gráfico, facilitando la elección según el objetivo.
